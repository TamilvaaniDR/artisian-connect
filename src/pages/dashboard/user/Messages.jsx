...
export default function Messages() {
  ...
  useEffect(() => {
    ...
    setThreads(all.filter(t => t.participants?.userId === u?.id));
  ...
  const filtered = useMemo(() => {
    ...
    return threads.filter((t) =>
      String(t.participants?.artisanId || "").includes(term) ||
  ...
  return (
    <div className="bg-white rounded-2xl shadow p-6">
      ...
      {loading ? (
        ...
        <EmptyState title="No messages yet" description="You will see your conversations with artisans here." />
      ) : filtered.length === 0 ? (
          ...
          {filtered.map((t) => (
            <div key={t.conversationId} className="border rounded-xl p-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs text-gray-500">Artisan #{t.participants?.artisanId}</span>
              </div>
              <div className="mt-2 space-y-1 max-h-48 overflow-auto">
                {t.messages?.map((m, idx) => (
                  <p key={idx} className="text-sm text-gray-800"><span className="font-semibold">{m.senderId === user?.id ? "You" : "Artisan"}:</span> {m.text}</p>
                ))}
              </div>
              <form
                onSubmit={(e) => {
                  ...
                  if (idx >= 0) {
                    ...
                    setThreads((prev) => prev.map((th) => (th.conversationId === t.conversationId ? all[idx] : th)));
